...
|`slug_field`|Значение (строка), которое возвращается методом `get_slug_field()`. По умолчанию "slug"|
...
|---|---|
|`get_context_data(request, **kwargs)`|[примесь `ContextMixin()`](#примесь%20`ContextMixin()`)|
|`get_object(queryset=None)`|Вызывается из класса `View`. Возвращает объект модели с первичным ключем `pk_url_kwarg` или `slug_url_kwarg`, значение которого ищется в `self.kwargs` (см. `setup()`)<br>Поиск объекта осуществляется в объекте, полученном при вызове методе через параметр `queryset`, а если он не передан, тогда поиск объекта осуществляется в 
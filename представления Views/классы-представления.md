...
|TemplateView||[`TemplateResponseMixin()`](#Примесь%20`TemplateResponseMixin()`), [`ContextMixin()`](#Примесь%20`ContextMixin()`), [`View()`](#класс%20`View()`)|
...
|`get(request, *args, **kwargs)`|Обработчик HTTP-метода GET.<br>Получает контекст, собранный с помощью `get_context_data()` и возвращает HTTP-ответ, подготовленный с помощью `render_to_response()`|

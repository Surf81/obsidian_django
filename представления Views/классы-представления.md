...
|---|---|
|`extra_context`|[примесь `ContextMixin()`](#примесь%20`ContextMixin()`)|
|`queryset`|Имеет приоритет перед `model`. Может быть указан объект типа `Manager()` или `QuerySet()` из которого `get_queryset()` будет возвращать все записи методом `all()`. По умолчанию `None`|
|`model`|Указывается ссылка на класс модели, из которой `get_queryset()` будет возращать все записи методом `_default_manager.all()` если не задан атрибут `queryset`. По умолчанию `None`|
|`slug_field`|По умолчанию "slug"|

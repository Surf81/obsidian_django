...
|`get_redirect_url(*args, **kwargs)`|Вызывается из обработчика метода HTTP-запроса (Например `get()`). Возвращает путь `url` с заполнеными шаблонами или `reverse()` на `pattern_name`, в конце которых добавлены ключи поичка если `query_string`|
|`get(request, *args, **kwargs)`, `post()`, `head()`, `options()`, `delete()`, `put()`, `patch()`|Обработчик метода HTTP-запрса. Вызывается из `dispatch()`, возвращает HTTP-ответ редиректа на адрес `url` или на маршрутизатор `pattern_name` в формате класса `HttpResponseRedirect()` или `HttpResponsePermanentRedirect()` (зависит от `permanent`)|
...
|`get(request, *args, **kwargs)`|Вызывается из `dispath()`. Обработчик метода HTTP-запроса `GET`.<br>Получает контекст, собранный с помощью `get_context_data()` и возвращает в `dispatch()` HTTP-ответ, подготовленный с помощью `render_to_response()`|
...
|---|---|
|`extra_context`|[примесь `ContextMixin()`](#примесь%20`ContextMixin()`)|
|`queryset`|Имеет приоритет перед `model`. Может быть указан объект типа `Manager()` или `QuerySet()` из которого `get_queryset()` будет возвращать все записи методом `all()`. По умолчанию `None`<br>Игнорируется если в `get_object()` передан queryset|
|`model`|Указывается ссылка на класс модели, из которой `get_queryset()` будет возращать все записи методом `_default_manager.all()`, если не задан атрибут `queryset`. По умолчанию `None`<br>Игнорируется если в `get_object()` передан queryset|
|`slug_field`|Значение (строка), которое возвращается методом `get_slug_field()`. По умолчанию "slug" - имя поля модели, в котором хранится слаг|
|`context_object_name`|Значение (строка), которое возвращается методом `get_context_object_name()`. По умолчанию `None`|
...
|метод|описание|
|---|---|
|`get_object(queryset=None)`|Вызывается из обработчика метода HTTP-запроса (Например `get()`). Возвращает объект модели с первичным ключем `pk_url_kwarg` или `slug_url_kwarg`, значение которого ищется в `self.kwargs` (см. `setup()`)<br>Поиск объекта осуществляется в объекте, полученном при вызове методе через параметр `queryset`, а если он не передан, тогда поиск объекта осуществляется в объекте `QuerySet()`, возвращенном методом `get_queryset()`|
|`get_queryset()`|Вызывается из `get_object()`, если в него не был передан queryset. Возвращает набор объектов из объекта `self.queryset.all()`, а если он не задан, то из `self.model._defautl_manager.all()`
|`get_slug_field()`|Вызывается из `get_object()`. Возвращает значение атрибута `self.slug_field` в котором хранится имя поля модели, в котором хранится слаг|
|`get_context_object_name(obj)`|Вызывается из `get_context_data()`. Задает в контексте альтернативное имя ключу `object`, в котором хранится экземпляр объекта. Альтернативным именем будет являться занчение, указанное в параметре `self.context_object_name`, а если оно не задано, тогда имя модели `obj._meta.model_name`|
|`get_context_data(request, **kwargs)`|Вызывается из методов `get()`, `post()` и прочих. Формирует справочник `context`, который затем будет передаваться в шаблон.<br>По умолчанию контекст содержит одно значение `view`, содержащее instance класса-контроллера. Так же в контекст добавляются значения `**kwargs` (из маршрутизатора) и `extra_context`<br>Если в экемпляре класса есть атрибут `self.object`, содержащий экземпляр объекта модели, то в контекст помещается значение `object`, а так же альтернативный ключ, возвращаемый `get_context_object_name()`||
```python
context = {
	'view': self,

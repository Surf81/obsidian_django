...
|`url`|Строка с URL-адресом, куда будет осуществляться редирект. Может содержать текстовые шаблоны `'%(key_name)s'` куда будут подставляться `**kwargs` из `По умолчанию `None`|
